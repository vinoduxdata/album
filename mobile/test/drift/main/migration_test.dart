  ...
  group('gallery-fork: from23To24 data migration', () {
    test('preserves existing v23 data and materializes new v24 objects', () async {
      ...
      final userCount = await migratedDb
          .customSelect("SELECT COUNT(*) AS c FROM user_entity WHERE id = 'user-pre-upgrade'")
          .getSingle();
      expect(userCount.data['c'], 1);

      // 2. Existing asset row survives.
      final assetCount = await migratedDb
          .customSelect("SELECT COUNT(*) AS c FROM remote_asset_entity WHERE id = 'asset-pre-upgrade'")
          .getSingle();
      ...
      final indexRows = await migratedDb
          ...
          .get();
      expect(indexRows, hasLength(1), reason: 'idx_remote_asset_library_created must exist after upgrade');

      // 5. PRAGMA foreign_key_check: no broken FKs anywhere.
      await migratedDb.customStatement('PRAGMA foreign_keys = ON');
      final fkErrors = await migratedDb.customSelect('PRAGMA foreign_key_check').get();
      expect(fkErrors, isEmpty, reason: 'foreign_key_check should be empty post-upgrade');
    ...
    test('populated library_id on a v23 asset survives the upgrade unchanged', () async {
      ...
      final row = await migratedDb
          .customSelect("SELECT library_id FROM remote_asset_entity WHERE id = 'lib-asset'")
          .getSingle();
      ...
      expect(
        ...
        reason: 'query planner should use the new composite index for (library_id, created_at DESC) lookups',
      );
